...
the six-stage rollout journey
(Package → Deploy → Configure & Integrate → Validate → Go Live → Operate & Scale), and
explicitly surfaces the **open design considerations** that still need input, so nothing
is silently assumed.

## Phase 0 — Data pack (prerequisite)

The following inputs are required before sizing/capacity decisions in Phase 2 can be
finalized:
...
- Run a formal STRIDE-based threat model workshop for the RAG/agent pipeline (still an
  open item) and use its output to scope the pre-GA pen test (also still open).
...
Phases 1–2 remove the structural risks of running everything in one VM (single point of
failure, blast radius, no environment separation, change risk, evidence gap) as fast as
possible. Phase 3 is what actually makes the platform *multi-tenant* rather than just
*re-hosted*. Phase 4 is what makes it *enterprise-sellable* (SLA, DR, compliance story)
...
- Landing zone pattern confirmation for a single-product estate (this document proposes
  one — see [03-platform-architecture.md](03-platform-architecture.md) §1 — pending
  sign-off)
